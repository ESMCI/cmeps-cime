module med_diag_mod
  ...
  private :: med_diag_print_level1
  private :: med_diag_print_level2
  private :: med_diag_print_level3
  ...
  real(r8),public :: budget_counter    (f_size,c_size,period_ntypes) ! counter, valid only on root pe
...
!===============================================================================
  ...
  subroutine med_phases_diag_ocn( gcomp, rc)
    ...
    if ( fldchk(is_local%wrap%FBMed_aoflux_o, 'Foxx_lwup', rc=rc) .and. &
       ...
       do n = 1,size(Foxx_lwup)
          ...
          budget_local(f_hlwup,ic,ip) = budget_local(f_hlwup,ic,ip) + (wgt_o+wgt_i)*Foxx_lwup(n)
          budget_local(f_hlatv,ic,ip) = budget_local(f_hlatv,ic,ip) + (wgt_o+wgt_i)*Foxx_lat(n)
          budget_local(f_hsen ,ic,ip) = budget_local(f_hsen ,ic,ip) + (wgt_o+wgt_i)*Foxx_sen(n)
          budget_local(f_wevap,ic,ip) = budget_local(f_wevap,ic,ip) + (wgt_o+wgt_i)*Foxx_evap(n)
       end do
    endif
  ...
  subroutine med_phases_diag_print(gcomp, rc)
    ...
    do ip = 1,period_ntypes
       ...
       ! Currently output_level is limited to levels of 0,1,2, 3 (see comment for print obtains at top)
       if (output_level > 0) then
          ...
          if (masterproc) then
             if (output_level >= 3) then
                ...
                call med_diag_print_level3(dataGrp)
             end if
             if (output_level >= 2) then
                ! detail lnd/ocn/ice component budgets ----
                call med_diag_print_level2(dataGrp)
             end if
             if (output_level >= 1) then
                ! net summary budgets
                call med_diag_print_level1(dataGrp)
             endif
             write(logunit,*) ' '
          endif

       endif  ! output_level > 0
  ...
  subroutine med_diag_print_level3(data)
    ...
    ! intput/output variables
    real(r8), intent(in) :: data(f_size,c_size,period_ntypes) ! values to print, scaled and such
    ...
    do ic = 1,2
       ...
       do nf = f_a, f_a_end
          write(logunit,FA1) fname(nf),&
               ...
               data(nf,ics,ip), &
               data(nf,ico,ip), &
               data(nf,ica,ip) + data(nf,icl,ip) + &
               data(nf,icn,ip) + data(nf,ics,ip) + data(nf,ico,ip)
       enddo
       ...
       write(logunit,FA1)    '   *SUM*'   ,&
            sum(data(f_h:f_h_end,ica,ip)), &
            sum(data(f_h:f_h_end,icl,ip)), &
            sum(data(f_h:f_h_end,icn,ip)), &
            sum(data(f_h:f_h_end,ics,ip)), &
            sum(data(f_h:f_h_end,ico,ip)), &
            sum(data(f_h:f_h_end,ica,ip)) + sum(data(f_h:f_h_end,icl,ip)) + &
            sum(data(f_h:f_h_end,icn,ip)) + sum(data(f_h:f_h_end,ics,ip)) + sum(data(f_h:f_h_end,ico,ip))
       ...
       enddo
       write(logunit,FA1)    '   *SUM*'   ,&
            sum(data(f_w:f_w_end,ica,ip)), &
            sum(data(f_w:f_w_end,icl,ip)), &
            sum(data(f_w:f_w_end,icn,ip)), &
            sum(data(f_w:f_w_end,ics,ip)), &
            sum(data(f_w:f_w_end,ico,ip)), &
            sum(data(f_w:f_w_end,ica,ip)) + sum(data(f_w:f_w_end,icl,ip)) + &
            sum(data(f_w:f_w_end,icn,ip)) + sum(data(f_w:f_w_end,ics,ip)) + sum(data(f_w:f_w_end,ico,ip))
       ...
       if ( flds_wiso ) then
          do is = 1, nisotopes
             ...
             write(logunit,FA1)    '   *SUM*', &
                  ...
                  sum(data(iso0(is):isof(is),icn,ip)) + sum(data(iso0(is):isof(is),ics,ip)) + sum(data(iso0(is):isof(is),ico,ip))
          end do
       end if

    enddo

  end subroutine med_diag_print_level3

  !===============================================================================

  subroutine med_diag_print_level2(data)
    ...
    do ic = 1,4
       ...
       enddo
       write(logunit,FA1)'   *SUM*',&
            -sum(data(f_h:f_h_end,icar,ip)), &
             sum(data(f_h:f_h_end,icxs,ip)), &
             sum(data(f_h:f_h_end,icxr,ip)), &
            -sum(data(f_h:f_h_end,icas,ip)), &
            -sum(data(f_h:f_h_end,icar,ip)) + sum(data(f_h:f_h_end,icxs,ip)) + &
             sum(data(f_h:f_h_end,icxr,ip)) - sum(data(f_h:f_h_end,icas,ip))
       write(logunit,*) ' '
       write(logunit,FAH) subname,trim(str)//' WATER BUDGET (kg/m2s*1e6): period = ',trim(pname(ip)),': date = ',cdate,curr_tod
       write(logunit,FA0) cname(icar),cname(icxs),cname(icxr),cname(icas),' *SUM*  '
       do nf = f_w, f_w_end
       ...
       enddo
       write(logunit,FA1)    '   *SUM*',&
            -sum(data(f_w:f_w_end,icar,ip)), &
             sum(data(f_w:f_w_end,icxs,ip)), &
             sum(data(f_w:f_w_end,icxr,ip)), &
            -sum(data(f_w:f_w_end,icas,ip)), &
            -sum(data(f_w:f_w_end,icar,ip)) + sum(data(f_w:f_w_end,icxs,ip)) + &
             sum(data(f_w:f_w_end,icxr,ip)) - sum(data(f_w:f_w_end,icas,ip))
  ...
  end subroutine med_diag_print_level2

  !===============================================================================

  subroutine med_diag_print_level1(data)
    ...
    do nf = f_a,f_a_end
       write(logunit,FA1) fname(nf),&
            data(nf,c_atm_arecv,ip), &
            data(nf,c_lnd_lrecv,ip), &
            data(nf,c_ocn_orecv,ip), &
            data(nf,c_inh_irecv,ip), &
            data(nf,c_ish_irecv,ip), &
            data(nf,c_atm_arecv,ip) + data(nf,c_lnd_lrecv,ip) + data(nf,c_ocn_orecv,ip) + &
            data(nf,c_inh_irecv,ip) + data(nf,c_ish_irecv,ip)
    enddo
    ...
    do nf = f_h, f_h_end
       write(logunit,FA1r)   fname(nf),&
            data(nf,c_atm_arecv,ip) + data(nf,c_atm_asend,ip), &
            data(nf,c_lnd_lrecv,ip) + data(nf,c_lnd_lsend,ip), &
            data(nf,c_rof_rrecv,ip) + data(nf,c_rof_rsend,ip), &
            data(nf,c_ocn_orecv,ip) + data(nf,c_ocn_osend,ip), &
            data(nf,c_inh_irecv,ip) + data(nf,c_inh_isend,ip), &
            data(nf,c_ish_irecv,ip) + data(nf,c_ish_isend,ip), &
            data(nf,c_glc_grecv,ip) + data(nf,c_glc_gsend,ip), &
            data(nf,c_atm_arecv,ip) + data(nf,c_atm_asend,ip) + &
            data(nf,c_lnd_lrecv,ip) + data(nf,c_lnd_lsend,ip) + &
            data(nf,c_rof_rrecv,ip) + data(nf,c_rof_rsend,ip) + &
            data(nf,c_ocn_orecv,ip) + data(nf,c_ocn_osend,ip) + &
    ...
    write(logunit,FA1r)'   *SUM*',&
         sum(data(f_h:f_h_end,c_atm_arecv,ip)) + sum(data(f_h:f_h_end,c_atm_asend,ip)), &
         sum(data(f_h:f_h_end,c_lnd_lrecv,ip)) + sum(data(f_h:f_h_end,c_lnd_lsend,ip)), &
         sum(data(f_h:f_h_end,c_rof_rrecv,ip)) + sum(data(f_h:f_h_end,c_rof_rsend,ip)), &
         sum(data(f_h:f_h_end,c_ocn_orecv,ip)) + sum(data(f_h:f_h_end,c_ocn_osend,ip)), &
         sum(data(f_h:f_h_end,c_inh_irecv,ip)) + sum(data(f_h:f_h_end,c_inh_isend,ip)), &
         sum(data(f_h:f_h_end,c_ish_irecv,ip)) + sum(data(f_h:f_h_end,c_ish_isend,ip)), &
         sum(data(f_h:f_h_end,c_glc_grecv,ip)) + sum(data(f_h:f_h_end,c_glc_gsend,ip)), &
         sum(data(f_h:f_h_end,c_atm_arecv,ip)) + sum(data(f_h:f_h_end,c_atm_asend,ip)) + &
         sum(data(f_h:f_h_end,c_lnd_lrecv,ip)) + sum(data(f_h:f_h_end,c_lnd_lsend,ip)) + &
         sum(data(f_h:f_h_end,c_rof_rrecv,ip)) + sum(data(f_h:f_h_end,c_rof_rsend,ip)) + &
         sum(data(f_h:f_h_end,c_ocn_orecv,ip)) + sum(data(f_h:f_h_end,c_ocn_osend,ip)) + &
    ...
    do nf = f_w, f_w_end
       write(logunit,FA1r)   fname(nf),&
            ...
            data(nf,c_ish_irecv,ip) + data(nf,c_ish_isend,ip), &
            data(nf,c_glc_grecv,ip) + data(nf,c_glc_gsend,ip), &
            data(nf,c_atm_arecv,ip) + data(nf,c_atm_asend,ip) + data(nf,c_lnd_lrecv,ip) + data(nf,c_lnd_lsend,ip) + &
            data(nf,c_rof_rrecv,ip) + data(nf,c_rof_rsend,ip) + data(nf,c_ocn_orecv,ip) + data(nf,c_ocn_osend,ip) + &
            data(nf,c_inh_irecv,ip) + data(nf,c_inh_isend,ip) + data(nf,c_ish_irecv,ip) + data(nf,c_ish_isend,ip) + &
            data(nf,c_glc_grecv,ip) + data(nf,c_glc_gsend,ip)
    enddo
    write(logunit,FA1r)'   *SUM*',&
         sum(data(f_w:f_w_end,c_atm_arecv,ip)) + sum(data(f_w:f_w_end,c_atm_asend,ip)),  &
         sum(data(f_w:f_w_end,c_lnd_lrecv,ip)) + sum(data(f_w:f_w_end,c_lnd_lsend,ip)),  &
         sum(data(f_w:f_w_end,c_rof_rrecv,ip)) + sum(data(f_w:f_w_end,c_rof_rsend,ip)),  &
         sum(data(f_w:f_w_end,c_ocn_orecv,ip)) + sum(data(f_w:f_w_end,c_ocn_osend,ip)),  &
         sum(data(f_w:f_w_end,c_inh_irecv,ip)) + sum(data(f_w:f_w_end,c_inh_isend,ip)),  &
         sum(data(f_w:f_w_end,c_ish_irecv,ip)) + sum(data(f_w:f_w_end,c_ish_isend,ip)),  &
         sum(data(f_w:f_w_end,c_glc_grecv,ip)) + sum(data(f_w:f_w_end,c_glc_gsend,ip)),  &
         sum(data(f_w:f_w_end,c_atm_arecv,ip)) + sum(data(f_w:f_w_end,c_atm_asend,ip)) + &
         sum(data(f_w:f_w_end,c_lnd_lrecv,ip)) + sum(data(f_w:f_w_end,c_lnd_lsend,ip)) + &
         sum(data(f_w:f_w_end,c_rof_rrecv,ip)) + sum(data(f_w:f_w_end,c_rof_rsend,ip)) + &
         sum(data(f_w:f_w_end,c_ocn_orecv,ip)) + sum(data(f_w:f_w_end,c_ocn_osend,ip)) + &
    ...
    if ( flds_wiso ) then
       ...
       do is = 1, nisotopes
          ...
          enddo
          write(logunit,FA1r)'   *SUM*',&
               sum(data(iso0(is):isof(is), c_atm_arecv, ip)) + sum(data(iso0(is):isof(is), c_atm_asend, ip)),  &
               sum(data(iso0(is):isof(is), c_lnd_lrecv, ip)) + sum(data(iso0(is):isof(is), c_lnd_lsend, ip)),  &
               sum(data(iso0(is):isof(is), c_rof_rrecv, ip)) + sum(data(iso0(is):isof(is), c_rof_rsend, ip)),  &
               sum(data(iso0(is):isof(is), c_ocn_orecv, ip)) + sum(data(iso0(is):isof(is), c_ocn_osend, ip)),  &
               sum(data(iso0(is):isof(is), c_inh_irecv, ip)) + sum(data(iso0(is):isof(is), c_inh_isend, ip)),  &
               sum(data(iso0(is):isof(is), c_ish_irecv, ip)) + sum(data(iso0(is):isof(is), c_ish_isend, ip)),  &
               sum(data(iso0(is):isof(is), c_glc_grecv, ip)) + sum(data(iso0(is):isof(is), c_glc_gsend, ip)),  &
               sum(data(iso0(is):isof(is), c_atm_arecv, ip)) + sum(data(iso0(is):isof(is), c_atm_asend, ip)) + &
               sum(data(iso0(is):isof(is), c_lnd_lrecv, ip)) + sum(data(iso0(is):isof(is), c_lnd_lsend, ip)) + &
               sum(data(iso0(is):isof(is), c_rof_rrecv, ip)) + sum(data(iso0(is):isof(is), c_rof_rsend, ip)) + &
               sum(data(iso0(is):isof(is), c_ocn_orecv, ip)) + sum(data(iso0(is):isof(is), c_ocn_osend, ip)) + &
  ...
  end subroutine med_diag_print_level1
