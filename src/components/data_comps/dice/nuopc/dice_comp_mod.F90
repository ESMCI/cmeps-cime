...
!===============================================================================
  ...
  subroutine dice_comp_init(flds_i2o_per_cat, mpicom, compid, my_task, master_task, &
    ...
    do n = 1, lsize
       if (abs(mod(SDICE%grid%data%rattr(index_lon,n) - xc(n),360.0_R8)) > 1.e-5) then
          write(6,*)'ERROR: DICE n, lon(domain), lon(mesh) = ',n, SDICE%grid%data%rattr(index_lon,n),xc(n)
          write(6,*)'ERROR: DICE lon diff = ',abs(SDICE%grid%data%rattr(index_lon,n) -  xc(n)),' too large'
          call shr_sys_abort('ERROR: DICE lon diff too large')
       end if
